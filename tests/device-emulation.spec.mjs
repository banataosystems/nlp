...
for (const [name, descriptor] of profiles) {
  test.describe(`${name} device-class contract`, () => {
    test.use({ ...descriptor });

    for (const route of routes) {
      test(`${route}: stays inside the device viewport`, async ({ page }) => {
    ...
    test('Discovery fails closed, then restores a usable composer after acknowledgement', async ({ page }) => {
      ...
      expect(composerBox.x + composerBox.width).toBeLessThanOrEqual((await page.evaluate(() => window.innerWidth)) + 1);
    });
    ...
    test('contracted viewport keeps Discovery composer reachable', async ({ page }) => {
      await page.goto('http://127.0.0.1:4173/#/discovery');
