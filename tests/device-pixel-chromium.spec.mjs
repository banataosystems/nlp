...
test.use({ ...pixel, browserName: 'chromium' });

const routes = ['home', 'discovery', 'cockpit', 'client'];

for (const route of routes) {
  test(`${route}: stays inside the Pixel viewport`, async ({ page }) => {
...
test('Discovery fails closed, then restores a usable Pixel composer after acknowledgement', async ({ page }) => {
...
test('Cherry OS keeps exactly one Pixel judgment page active and readable', async ({ page }) => {
...
test('contracted Pixel viewport keeps Discovery composer reachable', async ({ page }) => {
  await page.goto('http://127.0.0.1:4173/#/discovery');
